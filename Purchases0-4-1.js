...
function buildInfoBlock(purchaseID, index) {
    ...
    database.collection("purchases").doc(purchaseID).get().then(function(doc) {
        ...
        const printInvoiceButton = createDOMElement('div', 'purchase-invoice-button', 'Print Purchase Invoice', purchaseBlockBottomMiddle)
...
function createProductSearchResult(results) {
    ...
    results.hits.forEach(function(hit, hitIndex) {
        ...
        const conditionOptions = ['Loose', 'Acceptable', 'Good', 'Excellent', 'New']
        const conditionPaths = ['loose', 'usedAcceptable', 'usedGood', 'usedExcellent', 'new']
